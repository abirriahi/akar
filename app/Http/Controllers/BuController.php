...
class BuController extends Controller
{
    ...
    public function ShowSingle($id , Bu $bu){
        ...
        if($buInfo->bu_status == 0){
            $messageTitle = " هذا العقار يتنظر موافقة الادارة";
            $messageBody = "                        العقار
                         $buInfo->bu_name
                        موجود لدينا ولكنه في انتظار موافقة الادارة عليه
                         يتم نشر العقار في مدة اقصاها
                        24
                        ساعة";
            return view('website.bu.noper' , compact('buInfo' , 'messageTitle' , 'messageBody'));
        }
        $same_rent = $bu->where('bu_status' , 1)->where('bu_rent' , $buInfo->bu_rent)->orderBy(DB::raw('RAND()'))->take(3)->get();
        $same_type = $bu->where('bu_status' , 1)->where('bu_type' , $buInfo->bu_type)->orderBy(DB::raw('RAND()'))->take(3)->get();
    ...
    public function userEditBullding($id , Bu $bu){
        ...
        if($user->id != $buInfo->user_id){
            $messageTitle = "هذا العقار لم تقم بضافته ";
            $messageBody = "
            العقار
            $buInfo->name

            لم تقم باضافته
            تم اضافته من خلال عضوية اخري
...
العقار
$buInfo->name
تم تفعيلة ولا يمكنك التعديل عليه حاليا في حال اردت التعديل عليه برجاء الذهاب الي
    ...
    public function changeStatus ($id , $status  ,Bu $bu){
        ...
        return Redirect::back()->withFlashMessage(' تم التعديل علي حالة العقار بنجاح ');
    }
